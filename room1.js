...
room_five_questions = [["Linux's Completely Fair Scheduler adjusts timeslices based on the number of processes that are running. When there are a large number of processes running, this adjustment is likely to improve... \na) the mean wait time experienced by processes \nb) the compute throughput of the system \nc) the fairness of the amounts of processor time given to each process over a long period of time \nd)  the amount of stack space required in the kernel", "a"],["The XV6 scheduler scans the proc table (after first getting a lock) looking for a ready to run process. This is both slow and non scalable to thousands of processes. An alternative that is faster is to: \na) Randomly select a task to run and run it. \nb) Maintain a linked list of ready to run processes and always run the first process on the list, and place newly ready processes at the end. \nc) Maintain a hash table or ready to run processes keyed on their priority. \nd) Maintain a stack of proc table pointers and pop process to run them and push them when they become ready to run.", "b"]]
answer = "";
dir = "left";
